...
def of_l1_loss(
    pred_ofsts, kp_targ_ofst, labels, sigma=1.0, normalize=True, reduce=False
):
    """
    ...
    if normalize:
        in_loss = torch.sum(in_loss.view(bs, n_kpts, -1), 2) / (
...
class OFLoss(_Loss):
    ...
    def forward(self, pred_ofsts, kp_targ_ofst, labels, normalize=True, reduce=False):
        l1_loss = of_l1_loss(
            pred_ofsts, kp_targ_ofst, labels, sigma=1.0, normalize=True, reduce=False
        )
...
class CosLoss(_Loss):
    ...
    def forward(self, pred_ofsts, kp_targ_ofst, labels, normalize=True):
        ...
        kp_targ_ofst = kp_targ_ofst.permute(0, 2, 1, 3).contiguous()
        targ_vec = kp_targ_ofst / (
            torch.norm(kp_targ_ofst, dim=3, keepdim=True) + self.eps
        )
...
class BerHuLoss(nn.Module):
    ...
    def forward(self, pred, gt):
        ...
        mask = diff > threshold
        diff[mask] = ((img1[mask] - img2[mask]) ** 2 + threshold ** 2) / (
            2 * threshold + self.eps
        )
...
class PcldSmoothL1Loss(nn.Module):
    ...
    def dpt_2_pcld(self, dpt, K, xmap, ymap, scale2m=1.0):
        ...
        dpt_3d = torch.cat(
            [row.unsqueeze(-1), col.unsqueeze(-1), dpt.unsqueeze(-1)], dim=3
        )
        return dpt_3d
