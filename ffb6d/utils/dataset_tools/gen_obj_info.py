...
from utils import ImgPcldUtils, MeshUtils, SysUtils

parser = ArgumentParser()
parser.add_argument("--obj_name", type=str, default="ape", help="Object name.")
parser.add_argument(
    "--obj_pth", type=str, default="example_mesh/ape.ply", help="path to object ply."
)
...
parser.add_argument('--vis', action="store_true", help="visulaize rendered images.")
parser.add_argument(
    '--h', type=int, default=480, help="height of rendered RGBD images."
)
parser.add_argument('--w', type=int, default=640, help="width of rendered RGBD images.")
parser.add_argument(
    '--K',
    type=int,
...
parser.add_argument(
    '--n_latitude', type=int, default=3, help="number of latitude on sphere to sample."
)
parser.add_argument(
    '--extractor', type=str, default="ORB", help="2D keypoint extractor, SIFTO or ORB"
)
parser.add_argument(
    '--n_keypoint', type=int, default=8, help="number of keypoints to extract."
)
parser.add_argument(
    '--textured_3dkps_fd',
    type=str,
...
parser.add_argument(
    '--use_pyrender', action='store_true', help="use pyrender or raster_triangle"
)
parser.print_help()
...
def gen_one_mesh_info(args, obj_pth, sv_fd):
    ...
    textured_fps_pth = os.path.join(
        sv_fd, "%s_%s_fps.txt" % (args.obj_name, args.extractor)
    )
